...
public class QaAgentService {
    ...
    public AssistantResponse handle(String userInput, String sessionId) {
        String topic = extractTopic(userInput);
        String aiReply = llmClient.generateResponse(topic, List.of(
            Map.of("role", "system", "content", "你是智链工采的AI助手，专注于采购、供应链、比价、推荐、流程提醒等企业服务。你可以用专业、友好的语气为用户解答采购相关问题、推荐供应商、生成采购单、做采购统计等。"),
            Map.of("role", "user", "content", String.valueOf(userInput))
        ));
        AssistantResponse response = new AssistantResponse();
        response.setType(AssistantResponse.ResponseType.TEXT);
        response.setTitle("智能问答");
        response.setContent(aiReply);
        return response;
    }

    public AssistantResponse handleProductQuery(String userInput, String sessionId) {
        return handle(userInput, sessionId);
    }
