...
public class DataAgentService {
    ...
    public AssistantResponse analyzePurchasingData(String query) {
        String topic = extractTopic(query);
        String aiReply = llmClient.generateResponse(topic, List.of(
            Map.of("role", "system", "content", "你是智链工采的AI助手，专注于采购、供应链、比价、推荐、流程提醒等企业服务。你可以用专业、友好的语气为用户分析采购数据、生成统计报表。"),
            Map.of("role", "user", "content", String.valueOf(query))
        ));
        AssistantResponse response = new AssistantResponse();
        response.setType(AssistantResponse.ResponseType.CHART);
        response.setTitle("采购统计分析");
